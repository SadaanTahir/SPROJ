...
def moveglobal(conn,lat,long):
    ...
    conn.mav.send(mavutil.mavlink.MAVLink_set_position_target_global_int_message(10, conn.target_system,conn.target_component, 
                  ...
                  int(lat * 10 **7 ),int(long * 10 **7), 3, 0, 0, 0, 0, 0, 0, 0, 0))
...
# conn.mav.send(mavutil.mavlink.MAVLink_set_position_target_global_int_message(10, conn.target_system,conn.target_component, 
#                   mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, int(0b110111111000), int(-35.3629849 * 10 ** 7), int(149.1649185 * 10 ** 7), 10, 0, 0, 0, 0, 0, 0, 1.57, 0.5))
...
def moveupdown(conn):
    ...
    for i in range(3):
        conn.mav.send(mavutil.mavlink.MAVLink_set_position_target_local_ned_message(10, conn.target_system,conn.target_component, 
                        mavutil.mavlink.MAV_FRAME_LOCAL_NED, mask, 0, 0, prevheight-1, 0, 0, 0, 0, 0, 0, 0, 0))
        time.sleep(3)
        conn.mav.send(mavutil.mavlink.MAVLink_set_position_target_local_ned_message(10, conn.target_system,conn.target_component, 
                        mavutil.mavlink.MAV_FRAME_LOCAL_NED, mask, 0, 0, prevheight, 0, 0, 0, 0, 0, 0, 0, 0))
        time.sleep(3)
        conn.mav.send(mavutil.mavlink.MAVLink_set_position_target_local_ned_message(10, conn.target_system,conn.target_component, 
                        mavutil.mavlink.MAV_FRAME_LOCAL_NED, mask, 0, 0, prevheight, 0, 0, 0, 0, 0, 0, 0, 0))
