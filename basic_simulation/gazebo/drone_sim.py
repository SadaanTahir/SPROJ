...
def traverse(starting_points, division_lines, boundary_points, N):
  ...
  tmp = [[0,0]*4]
  print("start ... ",start)
  print("need to have separate moves for all drones, as some might stop to catch a flag while others may keep continuing")
  print(geofence[3])
  print()
  ...
  while True:
    ...
    #   if thismove[i] in ["forward","backward","left","right"]:
    #     print(f"moving {thismove[i]} drone ",i)
    #     movewrtlocal(connections[i], thismove[i], prev[i])
    #     #updating prev local location of i
    #     px, py = prev[i]    
    #     if thismove[i] == "forward":
    #       px-=2
    #     elif thismove[i] == "backward":
